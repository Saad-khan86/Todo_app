                    ...
                    <div className="hidden bg-slate-950 p-8 text-white lg:flex lg:flex-col lg:justify-between">
                        <div>
                            ...
                            <p className="mt-4 max-w-md text-sm leading-6 text-slate-300">
                                Sign in to organize tasks, track progress, and keep your workday moving.
                            </p>
                        </div>

                        <div className="grid gap-3 text-sm text-slate-300">
                            <div className="rounded-xl border border-white/10 bg-white/5 p-4">
                                Plan your priorities with a clean task list.
                            </div>
                            <div className="rounded-xl border border-white/10 bg-white/5 p-4">
                                Update and manage your todos from one place.
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="flex items-center justify-center p-5 sm:p-8 lg:p-10">
                        <form action={formAction} className="flex w-full max-w-md flex-col gap-5">
                            ...
                            <div className="space-y-2 text-center sm:text-left">
                                <p className="text-sm font-medium uppercase tracking-[0.18em] text-emerald-600">Account login</p>
                                <h1 className="text-3xl font-bold text-slate-950 sm:text-4xl">Login</h1>
                            ...
                            <div><p
                                ...
                                >
                                           If you do not have account please <Link className='font-semibold text-emerald-600 transition hover:text-emerald-700' href='/register'>Register</Link>
                         </p></div>

                            {/* Optional message */}
                            {message && (
                                <p
                                    className={`rounded-lg border px-3 py-2 text-sm ${status === 'success' ? 'border-green-200 bg-green-50 text-green-700' : 'border-red-200 bg-red-50 text-red-700'
                                        }`}
                                >
                                    {message}
                                </p>
